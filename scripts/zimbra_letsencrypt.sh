...
## functions
# check if certbot-auto / certbot / letsencrypt is installed and executable
check_executable () {
    # check for certbot or certbot-auto
    EXEC_BIN=`which certbot-auto`
    if [ -z "$EXEC_BIN" ]; then
        EXEC_BIN=`which certbot`
    fi

    # check if letsencrypt binary is present
    if [ -z "$EXEC_BIN" ]; then
        EXEC_BIN=`which letsencrypt`
    fi
    #  Exit if none is found
    if [ -z "$EXEC_BIN" ]; then
        echo "certbot or letsencrypt binary found not found in$PATH";
        echo -en "Do you want to install certbot-auto (Y/N)?"
        read selection
        if [[ $selection == "Y" || $selection == "y" ]]; then
            wget https://dl.eff.org/certbot-auto -P /usr/local/bin
            chmod a+x /usr/local/bin/certbot-auto
            EXEC_BIN="/usr/local/bin/certbot-auto"
        else
            echo "okay!, quiting now"
            exit 1;
        fi
    fi
}
...
request_certificate() {
    ...
    if [ -z "$DOMAIN" ]; then
        ...
        case $ZMODE in
            https)
                $EXEC_BIN certonly -a standalone --standalone-supported-challenges http-01 -d $DOMAIN
                ;;
            *)
                $EXEC_BIN certonly -a webroot -w $WEBROOT -d $DOMAIN
                ;;
        esac
        if [ $? -ne 0 ] ; then
...
# Test cert. 8.6 and below must use root<Plug>(neosnippet_expand)/
if version_gt $DETECTED_ZIMBRA_VERSION 8.7; then
