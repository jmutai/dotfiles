...
# Create some dirs
mkdir -p  ~/{.ncmpcpp,.mpd} 2>/dev/null 
mkdir -p  ~/.mpd/playlists 2>/dev/null
...
ask "Install Fedora Silverblue sway & packages?" Y && bash install-scripts/fedora-silverblue.sh
...
# Sway / Waybar
ask "Install Symlink for .config/sway?" Y && ln -sfn ${dir}/.config/sway ${HOME}/.config/sway
ask "Install Symlink for .config/waybar?" Y && ln -sfn ${dir}/.config/waybar ${HOME}/.config/waybar
ask "Install Symlink for .config/i3status?" Y && ln -sfn ${dir}/.config/i3status ${HOME}/.config/i3status
